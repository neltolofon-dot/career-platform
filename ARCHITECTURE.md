...
| D26 | Couche mouvement : CSS scroll-driven + un seul composant client (`MotionLayer`) qui écoute le pointeur et pilote le dock | aucune section publique ne passe en client ; le navigateur fournit déjà les API (coût mesuré : 2 à 3 points Lighthouse, cf. `docs/16-MOTION-PORTRAIT.md`) | GSAP ScrollTrigger (~70 Ko) ; Framer Motion (sections en `"use client"`) |
| D27 | Disque du portrait sombre à liseré vert, pas vert plein | un disque vert de 480 px serait l'accent dominant du premier écran ; l'accent reste la lumière de contour de la photo | disque `#60DD00` plein |
